...
void* mm_malloc (size_t size) {
	...
	// search for a free block of size reqSize
	ptrFreeBlock = searchFreeList(reqSize);
	if (ptrFreeBlock == NULL) { //Request more space from memory to store the block
		requestMoreSpace(reqSize);
		ptrFreeBlock = searchFreeList(reqSize);
	}
	//Determine if blockSize is exactly reqSize(adjusted for alignment) and split if need be 

	blockSize = SIZE(ptrFreeBlock->sizeAndTags);
	if ( blockSize - reqSize >= 2*MIN_BLOCK_SIZE ) {
	//Split block if it is bigger than need be
		splitBlock = (BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
		blockSize -= reqSize;
		splitBlock->sizeAndTags = blockSize | TAG_PRECEDING_USED;
		splitBlock->sizeAndTags &= ~TAG_USED; //Unallocate block that is split off
		*((size_t*) UNSCALED_POINTER_ADD(splitBlock, blockSize- WORD_SIZE)) = splitBlock->sizeAndTags;
		insertFreeBlock(splitBlock);
		blockSize = reqSize;
	} 
	else { 
	//Set next block's tags to show that the current block is used
...
void mm_free (void *ptr) {
	...
	blockInfo->sizeAndTags &= ~TAG_USED; //Sets block to unused
	*((size_t*) UNSCALED_POINTER_ADD(blockInfo, payloadSize)) = blockInfo->sizeAndTags;
	followingBlock->sizeAndTags &=  ~TAG_PRECEDING_USED | TAG_USED; 
